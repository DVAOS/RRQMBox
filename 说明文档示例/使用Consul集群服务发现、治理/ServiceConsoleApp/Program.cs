...
namespace ServiceConsoleApp
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                //因为HttpTouchRpcService是企业版功能，所以需要启用限时测试。
                //当然该功能可以用HttpService和TcpTouchRpcService组合实现，区别就是需要监听两个端口。
    ...
    class MyServer : ServerProvider
    {
        ...
        [WebApi(HttpMethodType.GET,Route = "/api/health")]
    ...
    /// WS命令行执行
    /// </summary>
    class MyWebSocketCommand : WSCommandLinePlugin
    {
    ...
    /// WS收到数据等业务。
    /// </summary>
    class MyWebSocketPlug : WebSocketPluginBase
    {
